...
public class PanMode<S extends IControlSurface<C>, C extends Configuration> extends AbstractTrackMode<S, C>
{
    ...
    {
        super ("First Remote", surface, model, isAbsolute);
    }
    ...
    public void onKnobValue (final int index, final int value)
    {
        ...
        if (track == null)
            return;
        if (this.isAbsolute)
            track.setRemoteControl (0, value);
        else
            track.changeRemoteControl (0, value);
    }
    ...
    {
        final ITrack track = this.model.getCurrentTrackBank ().getItem (index);
        return track == null ? -1 : track.getRemoteControl (0);
    }
}
